...
def reset_env():
    ...
    env_reset = env.reset()
    state = torch.from_numpy(np.array(env_reset)).float()
    state1 = prepare_initial_state(state)
...
for i in tqdm(range(epochs)):
    ...
    print('------------sssss-------------')
    print(F.softmax(F.normalize(q_val_pred)))
    print(action)
    print('-----------eeeee--------------')

    for j in range(params['action_repeats']): #D
        state2, e_reward_, done, info, _ = env.step(action)
    ...
    if done:
        ...
        state1 = state2
    # print(i, len(replay.memory))
    if len(replay.memory) < params['batch_size']:
        # print('inside ___ ', len(replay.memory))
        continue
